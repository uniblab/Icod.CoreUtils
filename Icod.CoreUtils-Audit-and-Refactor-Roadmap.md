...
| Current engineering milestone | Post-Gate-G executable composition-root refactor — **COMPLETE** |
| Completed infrastructure milestone | Completion Gates E2 through E6, F1 through F4, P1, Completion Gate G in full, and the post-Gate-G executable composition-root refactor |
...
| Next engineering step | Any later CoreUtils engineering work begins under a new milestone; the completed refactor is subject only to normal merge and main-branch Release acceptance |
...
**G10C / Completion Gate G closure checkpoint (2026-08-27):** reconciled the final Gate G checklist and this engineering roadmap against the G10B dependency/isolation evidence. Every cross-repository production edge is a published neutral-package dependency, every production `ProjectReference` is repository-local, command suites do not depend on sibling command suites at runtime, neutral foundations do not depend on command suites, the final graph is acyclic, and current independent CI evidence is green for every audited repository. No G10B version-pin migration is required. Completion Gate G is complete.

**Post-Gate-G executable composition-root closure checkpoint (2026-08-28):** the executable-host refactor establishes `Program.cs` as the operating-process composition root and `Command` as the independently callable command-semantics boundary. Where process resources are part of a command's contract, production hosts now acquire and inject the appropriate text or raw standard resources, translate process cancellation or signals deliberately, and clean up process-wide handlers deterministically. Byte-preserving commands retain raw-stream fidelity, while signal-forwarding, ignore-interrupt, terminal-descriptor, and native child-handle cases retain explicit command-specific host policy rather than being forced through a generic wrapper. Production-code head `9e7356535e9d366cced10d0f29543fc729ca5a87` passed PR #175's Staging clean/restore/build/test matrix on `windows-latest`, `ubuntu-latest`, and `macos-latest`. This documentation closure changes no production behavior; the normal `main` Release matrix remains the post-merge acceptance check. The invariant is recorded in `Icod.CoreUtils-Architecture-and-Migration.md`.
...
For the extracted sibling-suite projects, use the corresponding upstream project as the primary authority.
...
16. G4 through G8 have removed all sibling-suite source boundaries from CoreUtils. No sibling-suite project remains co-resident; G9 removes any stale solution, output-path, packaging, CI, or documentation residue left by the staged extractions.
...
`ps` and `uptime` were implemented and stabilized as part of this historical batch. Their completed history remains recorded here. Batch 57 replaces the live historical `uptime` project with the procps-ng-owned `Icod.ProcPs.Uptime` implementation. Batch 62 stages the suite-correct `Icod.ProcPs.Ps` implementation and tests alongside the historical `ps` project; the historical project is retired only after the replacement is green on the required validation path.
...
### Historical in-solution Patch and E-series partial-order workstream

This workstream does not alter command-batch numbering. The detailed Patch architecture, conformance matrix, security model, test requirements, phase checklists, and future Patch ownership now live in the standalone [Icod.Patch development roadmap](https://github.com/uniblab/Icod.Patch/blob/main/Icod.Patch-Development-Roadmap.md); this roadmap retains the repository-wide historical ordering constraints and the points at which Patch consumed the E-series infrastructure.
...
Batch 62 implementation is complete and ready for repository validation. The suite-correct `Icod.ProcPs.Ps` project and `tests/ProcPs.Ps.Tests` are staged alongside the historical `ps` project for the required green validation before retirement, while preserving the lowercase `ps` assembly name in a suite-specific output path. The command is a presentation and compatibility layer over `Icod.ProcPs.Shared`: it consumes the shared process provider, personality resolver, reusable process-reporting field/alias catalog, system metrics, account resolution, namespace/container observations, and lightweight-task supplement provider rather than the former CoreUtils process-information abstraction. The shared supplement model now also exposes Linux `/proc/PID/status` signal/capability masks and `/proc/PID/attr/current` security labels so those procps fields have authoritative Linux data and remain explicitly unavailable elsewhere.

The Batch 62 command covers default and explicit process selection, quick PID order, procps/BSD personality behavior, user-defined fields and headings, aliases, sorting, process forests, lightweight-thread reporting, width/header controls, environment and command forms, terminal/user/group presentation, cgroup/container and namespace fields, signal/capability/security fields, start/elapsed/CPU-time/CPU-percent/memory-percent calculations, help/version/error/cancellation behavior, and large enumerations. Dedicated tests inject all output/error streams and include shared procfs-fixture coverage for the new Linux status/security observations. Full solution and required-runner validation remain the closure step; after the replacement is green, retire the historical `ps` project/tests as the final Batch 62 migration step. Detailed notes are recorded in `Icod.CoreUtils-Batch-62-ProcPs-Process-Reporting.md`.
...
Creation consumes the established `ReadOnlyPathTraversalEngine`; archive rewrites and regular-file extraction consume the E6 transactional-replacement boundary; external compression consumes the shared process executor and private temporary workspace. Because the .NET 10 `TarWriter` surface does not expose the later `TarWriterOptions`/`TarHardLinkMode` API, hard-link preservation is command-owned: stable traversal filesystem/entry identities identify repeated physical files and `Icod.Tar` emits explicit hard-link archive members after the first pathname. Append/update preserve historical duplicate member versions at the archive tail, while mutation of compressed archives is rejected.
...
This gate was deliberately last. By this point the Coreutils, Diffutils, Grep, Patch, Ed, Sed, selected UtilLinux commands, Tar, and ProcPs projects had supplied the shared consumer evidence needed to choose stable API and repository boundaries. The neutral foundations plus UtilLinux, Grep, Tar, ProcPs, DiffUtils, LineEditor, and Patch were extracted; G9 completed final CoreUtils cleanup and G10 recorded, verified, and closed the resulting architecture.

- [x] Inventory the retained Shared surface and actual consumers through the G3 ownership/consumer contraction, G9 mechanical repository sweep, and G10B final production-edge audit; the retained audits and final repositories are the authoritative inventory rather than a transient standalone inventory artifact.
...
- [x] Classify retained and migrated APIs as:
  - [x] published neutral-foundation contracts for cross-suite or intrinsically command-neutral mechanism;
  - [x] Coreutils/Fileutils/Textutils-specific contracts retained in `Icod.CoreUtils.Shared`;
  - [x] suite-specific contracts retained in the owning suite's repository-local Shared/engine project where required;
  - [x] command-local contracts unsuitable for a shared package.
...
- [x] Eliminate circular dependencies and verify that no neutral foundation has a production dependency on a command suite.
- [x] Build and test every final repository against published external packages plus its own repository-local project references rather than neighboring source trees; G10B records current independent CI/build-validation evidence for every audited repository.
- [x] Publish `Icod.CoreUtils-Architecture-and-Migration.md` explaining the final package boundaries, repository split, executable ownership, versioning, release/CI ownership, and replacement of transitional Shared dependencies.

**Completion Gate G closure checkpoint (2026-08-27):** G10A records the final architecture, G10B supplies the cross-repository dependency/isolation and current independent CI evidence, and G10C reconciles the final checklists against that evidence. No production cross-repository `ProjectReference`, command-suite runtime-package edge, neutral-foundation back-edge, or dependency cycle remains, and no G10B version-pin migration is required.

Completion Gate G established the published neutral-foundation layer, retained `Icod.CoreUtils.Shared` as the repository-local Coreutils/Fileutils/Textutils library, and completed the repository split.
...
17. For a ProcPs milestone, verify that common processor, process, signal, priority, waiting, timing, status, host, and terminal mechanics are consumed from the appropriate published neutral foundations rather than duplicated in `Icod.ProcPs.Shared`.
18. For Completion Gate G, verify every extracted repository against its applicable published neutral packages plus its own repository-local projects before declaring the architecture stable.
...
- ProcPs batches consume the published neutral processor, process, timing, host, and terminal foundations without duplicating their identities, targets, launch, wait, signal, priority, timing, status, host, or terminal contracts;
- Completion Gate G leaves every neutral foundation free of command-suite dependencies, preserves `Icod.CoreUtils.Shared` only where Coreutils/Fileutils/Textutils-specific reuse remains, and verifies all cross-repository consumers against published neutral packages.