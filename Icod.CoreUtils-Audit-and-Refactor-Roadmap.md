...
| Current engineering milestone | ProcPs three-platform provider correction implemented; Batch 57 and provider validation pending |
...
| Active infrastructure dependency | Validate the corrected Linux/Windows/macOS ProcPs providers before Batch 58 consumes the shared sampling and metric foundation |
| Next engineering step | Validate the ProcPs provider correction and Batch 57 on all required runners; then Batch 58 — `Icod.ProcPs.Vmstat` |
...
A post-Batch-57 three-platform provider correction is implemented and awaits the required runner validation. Linux remains the exact procps-ng profile. Windows now uses dedicated native providers for parent PIDs, Windows login/session identity, physical/available/cache/commit/pagefile memory, aggregate CPU activity, system uptime, and logged-in Terminal Services sessions. macOS now uses Darwin `libproc`, Mach, sysctl, `getloadavg()`, and POSIX `utmpx`/`getsid()` sources for parent/group/session/user/terminal/priority process metadata, physical and VM memory, swap, CPU activity, load averages, uptime, and logged-in users. The generic .NET providers are retained only as conservative fallbacks and no longer mislabel `Process.SessionId` as a POSIX process session. Neutral CPU/load/memory observations were added alongside the existing Linux-specific raw models so non-Linux providers need not fabricate Linux-only counters. Linux-only `vmstat`, slab, huge-page, namespace, container, and detailed `/proc/loadavg` fields remain explicitly unsupported where no defensible equivalent exists. `free` consumes the neutral memory fields and `uptime` consumes the neutral load-average observation, allowing their portable portions to use the new providers without weakening Linux fidelity. OS-gated tests cover the native Windows and macOS provider paths. Detailed correction notes are recorded in `Icod.CoreUtils-ProcPs-Three-Platform-Provider-Correction.md`. The implementation environment did not provide a .NET SDK, so Debug/Release builds and the `windows-latest`, `ubuntu-latest`, and `macos-latest` closure checks remain pending.
