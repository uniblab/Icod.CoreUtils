		...
		try {
			...
			if ( result.HasOption( "count" ) ) {
				...
				await foreach ( var record in provider.ReadAsync( fileName, context.CancellationToken ).ConfigureAwait( false ) ) {
					if ( currentTerminalOnly && !string.Equals( record.Line, currentTerminal, StringComparison.Ordinal ) )
						continue;
					if ( LoginRecordType.UserProcess == record.Type && !string.IsNullOrEmpty( record.User ) )
			...
			await foreach ( var record in provider.ReadAsync( fileName, context.CancellationToken ).ConfigureAwait( false ) ) {
				...
				var line = FormatRecord( record with {
					Host = host
				}, includeIdle, includeMesg );
