...
With no operands, `vdir` lists the current directory. It uses the same shared engine and option vocabulary as `ls`, but its profile defaults to long metadata output with escaped filename presentation.
...
`vdir(1)`, `ls(1)`, `dir(1)`, `dircolors(1)`, `stat(1)`
