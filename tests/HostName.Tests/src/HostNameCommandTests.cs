...
public sealed class HostNameCommandTests {
	...
	[Fact] public async Task FileSettingReadsAsynchronouslyThenFails() { var file=System.IO.Path.GetTempFileName(); try { await File.WriteAllTextAsync(file,"host-from-file\n"); Assert.Equal(1,await Tool.RunAsync(new[]{"-F",file},stderr:new StringWriter())); } finally { File.Delete(file); } }
