...
public sealed class PathChkCommandTests {
	...
	[Fact] public async Task NonexistentDefaultPathIsValid() => Assert.Equal(0,await Tool.RunAsync(new[]{Path.Combine(Path.GetTempPath(),Guid.NewGuid().ToString("N"),"file")}));
