...
public sealed class PwdCommandTests {
	[Fact] public async Task PhysicalPrintsCurrentDirectory() { var o=new StringWriter(); Assert.Equal(0,await Tool.RunAsync(new[]{"-P"},stdout:o)); Assert.Equal(Path.TrimEndingDirectorySeparator(Path.GetFullPath(Directory.GetCurrentDirectory())),o.ToString().TrimEnd()); }
	[Fact] public async Task LogicalUsesValidPwd() { var old=Environment.GetEnvironmentVariable("PWD"); try { var cwd=Path.GetFullPath(Directory.GetCurrentDirectory()); Environment.SetEnvironmentVariable("PWD",cwd); var o=new StringWriter(); Assert.Equal(0,await Tool.RunAsync(new[]{"-L"},stdout:o)); Assert.Equal(cwd,o.ToString().TrimEnd()); } finally { Environment.SetEnvironmentVariable("PWD",old); } }
	[Fact] public async Task InvalidLogicalPwdFallsBack() { var old=Environment.GetEnvironmentVariable("PWD"); try { Environment.SetEnvironmentVariable("PWD","relative/../bad"); var o=new StringWriter(); Assert.Equal(0,await Tool.RunAsync(new[]{"-L"},stdout:o)); Assert.True(Path.IsPathRooted(o.ToString().TrimEnd())); } finally { Environment.SetEnvironmentVariable("PWD",old); } }
