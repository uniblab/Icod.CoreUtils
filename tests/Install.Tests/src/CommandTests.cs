...
public sealed class CommandTests {
	...
	/// <summary>Verifies wildcard source operands expand while the destination remains a literal directory operand.</summary>
	[Fact]
	public async Task ExpandsSourcePatternIntoDestinationDirectory() {
		...
		try {
			...
			Assert.Equal(
				0,
				await RunAsync( new[] { System.IO.Path.Combine( root, "*.txt" ), destination } )
			);
