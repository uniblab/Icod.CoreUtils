...
namespace Icod.CoreUtils.Shared.Tests.FileSystem.Platform.Ubuntu;

/// <summary>Exercises Shared filesystem observations against native Ubuntu filesystem objects.</summary>
public sealed class UbuntuFileSystemLinkIntegrationTests {
	/// <summary>Verifies Ubuntu hard links share inode identity and link count without becoming pathname indirection.</summary>
	[Fact]
	public async Task ReportsUbuntuHardLinkIdentityAndCountWhenSupported() {
		if ( !IsUbuntuHost() ) {
			return;
		}
	...
	/// <summary>Verifies Ubuntu symbolic-link metadata preserves the physical link identity while following the target.</summary>
	[Fact]
	public async Task ReportsUbuntuSymbolicLinkObjectAndFollowedTargetWhenSupported() {
		if ( !IsUbuntuHost() ) {
			return;
		}
	...
	/// <summary>Verifies native Ubuntu FIFOs and Unix-domain sockets remain distinguishable path objects.</summary>
	[Fact]
	public async Task ClassifiesUbuntuFifoAndUnixDomainSocketWhenSupported() {
		if ( !IsUbuntuHost() ) {
			return;
		}
	...
	private static string CreateTemporaryDirectory() {
		var path = SystemPath.Combine(
			"/tmp",
			string.Concat( "icod-ubuntu-links-", Guid.NewGuid().ToString( "N" ) )
		);
