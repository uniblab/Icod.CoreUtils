...
public sealed class BinaryFormattingTests {
	...
	public void IntegerFormattingHonorsByteOrder() {
		...
		Assert.Equal( "0201", BinaryValueFormatter.Format( format, new byte[] { 1, 2 }, BinaryByteOrder.LittleEndian ) );
		Assert.Equal( "0102", BinaryValueFormatter.Format( format, new byte[] { 1, 2 }, BinaryByteOrder.BigEndian ) );
	}
	[Fact]
	public void OctalAndHexadecimalFormattingUseFixedPrecision() {
		var octal = new BinaryFormatSpecification( BinaryFormatKind.Octal, 2, false, "o2" );
		var hexadecimal = new BinaryFormatSpecification( BinaryFormatKind.Hexadecimal, 1, false, "x1" );
		Assert.Equal( "000001", BinaryValueFormatter.Format( octal, new byte[] { 1, 0 }, BinaryByteOrder.LittleEndian ) );
		Assert.Equal( "01", BinaryValueFormatter.Format( hexadecimal, new byte[] { 1 }, BinaryByteOrder.Native ) );
	}
	[Fact]
