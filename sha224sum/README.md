...
**sha224sum** — compute or check SHA-224 message digests
...
`Icod.CoreUtils.Sha224Sum` is a managed .NET implementation of GNU Coreutils `sha224sum(1)`, modeled on GNU Coreutils 9.11.

The command computes a SHA-224 message digest for each `FILE` and writes a checksum record to standard output. If no file is specified, or if a file operand is `-`, input is read from standard input. The command produces a 224-bit SHA-224 digest.
...
GNU `sha224sum` was written by Ulrich Drepper, Scott Miller, and David Madore.
...
`sha224sum(1)`, `cksum(1)`, `b2sum(1)`, `md5sum(1)`, `sha1sum(1)`, `sha256sum(1)`, `sha512sum(1)`