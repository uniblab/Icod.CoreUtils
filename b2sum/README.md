...
**b2sum** — compute or check BLAKE2 message digests
...
`Icod.CoreUtils.B2Sum` is a managed .NET implementation of GNU Coreutils `b2sum(1)`, modeled on GNU Coreutils 9.11.

The command computes a BLAKE2 message digest for each `FILE` and writes a checksum record to standard output. If no file is specified, or if a file operand is `-`, input is read from standard input. By default the command produces a 512-bit BLAKE2b digest. The digest size can be changed with `--length`.
