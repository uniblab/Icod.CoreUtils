...
**md5sum** — compute or check MD5 message digests
...
`Icod.CoreUtils.Md5Sum` is a managed .NET implementation of GNU Coreutils `md5sum(1)`, modeled on GNU Coreutils 9.11.

The command computes a MD5 message digest for each `FILE` and writes a checksum record to standard output. If no file is specified, or if a file operand is `-`, input is read from standard input. The command produces a 128-bit MD5 digest.
...
GNU `md5sum` was written by Ulrich Drepper, Scott Miller, and David Madore.
...
`md5sum(1)`, `cksum(1)`, `b2sum(1)`, `md5sum(1)`, `sha1sum(1)`, `sha256sum(1)`, `sha512sum(1)`