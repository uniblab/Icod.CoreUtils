...
""";
		...
		try {
			...
			if ( parsed.NameOnly && parsed.Quiet ) {
				await WriteDiagnosticAsync(
					stderr,
					"sysctl: options -N and -q cannot coexist\nTry 'sysctl --help' for more information.",
					cancellationToken
	...
	private static async Task<int> DisplayAllAsync(
		ISysctlBackend backend,
		...
		try {
			...
			await WriteDiagnosticAsync( stderr, $"sysctl: unable to open directory \"/proc/sys/\": {exception.Message}", cancellationToken ).ConfigureAwait( false );
			return Failure;
		}
		var status = Success;
	...
	private static async Task<int> RunPreloadAsync(
		ISysctlBackend backend,
		ParsedArguments options,
		Stream? stdin,
		Stream? stdout,
		...
		foreach ( var filePattern in files ) {
			...
			try {
				...
				await WriteDiagnosticAsync( stderr, $"sysctl: glob failed: {exception.Message}", cancellationToken ).ConfigureAwait( false );
				status |= Failure;
				continue;
			}
	...
	private static async Task<int> RunSystemAsync(
		ISysctlBackend backend,
		ParsedArguments options,
		Stream? stdout,
		Stream? stderr,
	...
	private static async Task<IReadOnlyList<string>> ResolveSystemConfigurationFilesAsync(
		ISysctlBackend backend,
		CancellationToken cancellationToken
	) {
		ArgumentNullException.ThrowIfNull( backend );
	...
	private static async Task<int> PreloadFileAsync(
		ISysctlBackend backend,
		string path,
		ICollection<SysctlSetting> settings,
		ParsedArguments options,
		Stream? stderr,
		CancellationToken cancellationToken
	) {
		...
		try {
			...
			await WriteDiagnosticAsync( stderr, $"sysctl: cannot open \"{path}\": {exception.Message}", cancellationToken ).ConfigureAwait( false );
			return Failure;
		}
		return await ParseConfigurationLinesAsync( path, lines, settings, options, stderr, cancellationToken ).ConfigureAwait( false );
	}

	private static async Task<int> PreloadStandardInputAsync(
		Stream? stdin,
		ICollection<SysctlSetting> settings,
		ParsedArguments options,
		Stream? stderr,
	...
	private static async Task<int> ParseConfigurationLinesAsync(
		string path,
		IReadOnlyList<string> lines,
		ICollection<SysctlSetting> settings,
		ParsedArguments options,
	...
	private static async Task<int> WriteSettingListAsync(
		ISysctlBackend backend,
		IReadOnlyList<SysctlSetting> settings,
		ParsedArguments options,
		Stream? stdout,
		Stream? stderr,
		CancellationToken cancellationToken
	...
	private static async Task<int> ReadSettingAsync(
		...
		if ( options.NameOnly ) {
			try {
				...
				return await ReportReadFailureAsync( key, displayKey, exception, options, stderr, cancellationToken ).ConfigureAwait( false );
			}
		}
		try {
			...
			return Success;
		} catch ( SysctlBackendException exception ) when ( SysctlBackendFailureKind.IsDirectory == exception.Kind ) {
			return await DisplaySubtreeAsync( backend, key, options, stdout, stderr, cancellationToken ).ConfigureAwait( false );
		} catch ( SysctlBackendException exception ) {
			return await ReportReadFailureAsync( key, displayKey, exception, options, stderr, cancellationToken ).ConfigureAwait( false );
		}
	...
	private static async Task<int> DisplaySubtreeAsync(
		ISysctlBackend backend,
		string prefix,
		ParsedArguments options,
		Stream? stdout,
	...
	private static async Task<int> ReportReadFailureAsync(
		string key,
		string displayKey,
		SysctlBackendException exception,
		ParsedArguments options,
		Stream? stderr,
		CancellationToken cancellationToken
	) {
	...
	private static async Task<int> WriteSettingAsync(
		ISysctlBackend backend,
	...
	private sealed record SysctlSetting(
		string PathKey,
		string DisplayKey,
		string? Value,
		bool IgnoreFailure,
