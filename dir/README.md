...
**dir** — list directory contents in columns
...
`Icod.CoreUtils.Dir` is a managed .NET implementation of GNU Coreutils `dir(1)`, modeled on GNU Coreutils 9.11.

With no operands, `dir` lists the current directory. It uses the same shared engine and option vocabulary as `ls`, but its profile defaults to terminal-width-aware column output and escaped filename presentation regardless of whether standard output is attached to a terminal.
