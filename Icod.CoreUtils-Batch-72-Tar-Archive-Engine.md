...
Creation uses the shared `ReadOnlyPathTraversalEngine`, honors recursion/dereference policy and exclusions, and preserves link typing through `System.Formats.Tar`. On .NET 10, `TarWriter` exposes the `(Stream, TarEntryFormat, bool)` constructor but not the newer `TarWriterOptions`/`TarHardLinkMode` surface; `Icod.Tar` therefore preserves filesystem hard-link identity itself by tracking the shared traversal layer's stable filesystem/entry identities and emitting explicit hard-link members after the first archived pathname. Archive publication and archive rewrites use the E6 transactional replacement boundary.
