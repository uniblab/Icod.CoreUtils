...
$stageDirectoryName = "Icod.CoreUtils-$Version-$RuntimeIdentifier"
...
$projects = Get-CoreUtilsProjectMap
$projects['coreutils'] = 'coreutils/Icod.CoreUtils.Router.csproj'
...
try {
    foreach ($commandName in $projects.Keys) {
        ...
        Copy-Item -LiteralPath $publishedExecutable -Destination (
            Join-Path $stageDirectory $executableFileName
        )

        foreach ($documentationFileName in @(
            "$commandName.README.md",
            "$commandName.LICENSE.txt"
        )) {
            $publishedDocumentation = Join-Path $publishDirectory $documentationFileName
            if (-not (Test-Path -LiteralPath $publishedDocumentation -PathType Leaf)) {
                throw "Publish did not produce '$publishedDocumentation'."
            }
            Copy-Item -LiteralPath $publishedDocumentation -Destination (
    ...
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'LICENSE') -Destination (
        Join-Path $stageDirectory 'LICENSE'
    )
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'README.md') -Destination (
