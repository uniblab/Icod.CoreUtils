...
public static class Command {
	...
	public static async Task<int> RunAsync( string[] args, CommandContext context ) {
		...
		try {
			...
			var failed = false;
			foreach ( var path in result.Operands ) {
				failed |= !await ValidateAsync(
					path,
					portable,
					leading,
					context
		...
		if ( leading ) {
			foreach ( var component in components ) {
				if ( component.Length > 0 && component[0] == '-' ) {
					await Bad(
						$"leading '-' in a component of file name '{path}'"
					).ConfigureAwait( false );
				}
			}
		}
		if ( portable ) {
			if ( Encoding.UTF8.GetByteCount( path ) > 255 ) {
				await Bad(
					$"limit 255 exceeded by length {Encoding.UTF8.GetByteCount( path )} of file name '{path}'"
				).ConfigureAwait( false );
			}
			foreach ( var component in components ) {
				if ( Encoding.UTF8.GetByteCount( component ) > 14 ) {
					await Bad(
						$"limit 14 exceeded by length {Encoding.UTF8.GetByteCount( component )} of file name component '{component}'"
					).ConfigureAwait( false );
				}
			}
			foreach ( var ch in path ) {
				if ( !IsPortable( ch ) ) {
					await Bad(
						$"nonportable character '{ch}' in file name '{path}'"
					).ConfigureAwait( false );
					break;
				}
			}
		} else {
			var pathLimit = ( OperatingSystem.IsWindows() )
				? 32767
				: 4095
			;
			if ( Encoding.UTF8.GetByteCount( path ) > pathLimit ) {
				await Bad(
					$"file name '{path}' is too long"
				).ConfigureAwait( false );
			}
			foreach ( var component in components ) {
				if ( Encoding.UTF8.GetByteCount( component ) > 255 ) {
					await Bad(
						$"file name component '{component}' is too long"
					).ConfigureAwait( false );
				}
			}
			if ( path.IndexOf( '\0' ) >= 0 ) {
	...
	private static bool IsPortable( char ch ) =>
		ch == '/'
		|| ch == '.'
		|| ch == '_'
		|| ch == '-'
