...
/// <remarks>
/// <para>Usage:</para>
/// <code>expand [OPTION]... [FILE]...</code>
/// <para>Tabs are expanded without normalizing untouched input bytes.</para>
/// </remarks>
public static class Command {
	private const string InitialKey = "initial";
	private const string TabsKey = "tabs";
	private const string LegacyTabsKey = "legacy-tabs";
	private const string HelpKey = "help";
	private const string VersionKey = "version";

	/// <summary>Runs <c>expand</c> synchronously with optional injected text streams.</summary>
	...
	/// <summary>Runs <c>expand</c> asynchronously with optional injected text streams.</summary>
	/// <param name="args">The command-line arguments.</param>
	/// <param name="standardInput">The standard-input reader.</param>
	/// <param name="standardOutput">The standard-output writer.</param>
	/// <param name="standardError">The standard-error writer.</param>
	...
	/// <summary>Runs <c>expand</c> asynchronously against a command context.</summary>
	/// <param name="args">The command-line arguments.</param>
	...
	public static async Task<int> RunAsync( string[] args, CommandContext context ) {
		...
		try {
			...
			var processor = new ExpandProcessor(
				options,
	...
	private static OptionParser CreateParser() {
		...
		return new OptionParser(
			new[] {
				new OptionDefinition( InitialKey, 'i', new[] { "initial" } ),
	...
	private static async Task WriteOptionErrorAsync( OptionParseError error, CommandContext context ) {
		...
		await context.StandardError.WriteLineAsync(
			"Try 'expand --help' for more information.".AsMemory(),
			context.CancellationToken
		).ConfigureAwait( false );
	}

	private static Task WriteUsageAsync( TextWriter writer, CancellationToken cancellationToken ) {
		return writer.WriteLineAsync( "Usage: expand [OPTION]... [FILE]...".AsMemory(), cancellationToken );
	}
	...
	private static async Task WriteHelpAsync( TextWriter writer, CancellationToken cancellationToken ) {
		...
		await writer.WriteAsync(
			string.Concat(
				"Convert tabs in each FILE to spaces, writing to standard output.", Environment.NewLine,
				"With no FILE, or when FILE is -, read standard input.", Environment.NewLine,
				Environment.NewLine,
				"  -i, --initial       do not convert tabs after non blanks", Environment.NewLine,
				"  -t, --tabs=LIST     use comma- or blank-separated tab positions", Environment.NewLine,
				"      --help          display this help and exit", Environment.NewLine,
				"      --version       output version information and exit", Environment.NewLine,
				Environment.NewLine,
				"After the last explicit position, /N repeats at multiples of N and +N repeats relative to the last stop.", Environment.NewLine
			).AsMemory(),
			cancellationToken
		).ConfigureAwait( false );
	}
	...
	private static Task WriteVersionAsync( TextWriter writer, CancellationToken cancellationToken ) {
		return writer.WriteLineAsync( "expand (Icod.CoreUtils) GNU Coreutils 9.11 compatibility profile".AsMemory(), cancellationToken );
	}
}
