...
/// <para>Usage: <c>ed [OPTION]... [[+LINE] FILE]</c>.</para>
/// </summary>
...
Edit text line by line.
  ...
  -r, --restricted        restrict filenames and disable shell commands
...
""";
	...
	private static async Task<int> RunCoreAsync(
		...
		try {
			...
			var options = EdOptions.From( parsed );
	...
	private static async Task<bool> LoadInitialFileAsync(
		...
		try {
			...
			if ( fileName.StartsWith( '!' ) ) {
				...
				var process = await processAccess.RunShellAsync(
					fileName[ 1.. ],
				...
				if ( 0 != ( process.ExitCode ?? 1 ) ) {
					throw new IOException( "The initial shell command failed." );
				}
				read = await ReadRecordsAsync(
					process.StandardOutput,
					options.StripTrailingCarriageReturn,
					cancellationToken
				).ConfigureAwait( false );
				engine.Load( read.Lines, read.FinalRecordTerminated );
			} else {
	...
	private static bool TryCreateInvocation(
		...
		invocation = new EdInvocation( initialAddress, fileName );
		error = null;
		return true;
	}
	...
	private static async ValueTask<EditorFileReadResult> ReadRecordsAsync(
		ReadOnlyMemory<byte> bytes,
		...
		while ( true ) {
			...
			if ( stripTrailingCarriageReturn && record.IsTerminated && !content.IsEmpty && (byte)'\r' == content.Span[ ^1 ] ) {
				content = content[ ..^1 ];
			}
			lines.Add( content.ToArray() );
...
""";
	...
	private sealed record EdInvocation(
		string? InitialAddress,
		string? FileName
	);

	private sealed record EdOptions(
