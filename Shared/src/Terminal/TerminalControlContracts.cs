...
public sealed class TerminalEndpoint {
	...
	/// <summary>Gets the file descriptor when <see cref="Kind"/> is <see cref="TerminalEndpointKind.FileDescriptor"/>.</summary>
	public int? FileDescriptor {
		get;
	}

	/// <summary>Gets the device pathname when <see cref="Kind"/> is <see cref="TerminalEndpointKind.Path"/>.</summary>
	public string? Path {
		get;
	}
	...
	public string DisplayName {
		get {
			return TerminalEndpointKind.FileDescriptor == this.Kind
				? string.Concat( "file descriptor ", this.FileDescriptor!.Value.ToString( System.Globalization.CultureInfo.InvariantCulture ) )
				: this.Path!;
		}
	}
	...
	/// <exception cref="ArgumentOutOfRangeException"><paramref name="fileDescriptor"/> is negative.</exception>
...
public sealed class TerminalControlResult<T> {
	...
	/// <exception cref="InvalidOperationException">The result is not available.</exception>
...
public sealed class TerminalModeSnapshot {
	...
	private TerminalModeSnapshot(
		...
		var characters = controlCharacters.ToArray();
		switch ( platform ) {
