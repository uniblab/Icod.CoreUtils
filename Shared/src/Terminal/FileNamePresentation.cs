...
public enum FileNameQuotingStyle {
	...
	/// <summary>Use locale-style quotation; the incubation profile uses single quotes.</summary>
	Locale
}
...
public sealed class FileNamePresentationPolicy {
	...
	/// </summary>
	/// <param name="quotingStyle">The quoting style.</param>
	/// <param name="controlCharacters">The control-character policy.</param>
	/// <exception cref="ArgumentOutOfRangeException">
	/// A supplied policy value is not defined.
	/// </exception>
	/// <exception cref="ArgumentException">
	/// Escape presentation was combined with a quoting style that does not
	...
	public FileNamePresentationPolicy(
		...
		if ( !Enum.IsDefined( quotingStyle ) ) {
		...
		if ( !Enum.IsDefined( controlCharacters ) ) {
			throw new ArgumentOutOfRangeException(
		...
		if ( ( ControlCharacterPresentation.Escape == controlCharacters )
			&& !UsesEscapeSyntax( quotingStyle ) ) {
	...
	/// <summary>
	/// Resolves the default listing policy from explicit values, the
	/// <c>QUOTING_STYLE</c> environment variable, and terminal attachment.
	/// </summary>
	/// <param name="presentation">The terminal presentation snapshot.</param>
	/// <param name="requestedStyle">An explicit command-line quoting style.</param>
	/// <param name="requestedControlCharacters">An explicit control-character policy.</param>
	/// <returns>The resolved immutable policy.</returns>
	/// <exception cref="ArgumentNullException">
	/// <paramref name="presentation"/> is <see langword="null"/>.
	/// </exception>
	/// <exception cref="ArgumentOutOfRangeException">
	/// An explicit policy value is not defined.
	/// </exception>
	/// <exception cref="ArgumentException">
	/// Explicit escape presentation was combined with a quoting style that
	/// does not provide escape syntax.
	/// </exception>
	...
	/// </summary>
	/// <param name="value">The candidate style name.</param>
	/// <param name="style">The parsed style.</param>
	...
	private static ControlCharacterPresentation DefaultControlCharacterPolicy(
		...
		if ( style is FileNameQuotingStyle.C
			or FileNameQuotingStyle.CMaybe
			or FileNameQuotingStyle.Escape
			or FileNameQuotingStyle.CLocale
			or FileNameQuotingStyle.Locale
...
public static class FileNamePresenter {
	...
	/// </summary>
	/// <param name="value">The filename to present.</param>
	/// <param name="policy">The resolved presentation policy.</param>
	/// <returns>The presented filename.</returns>
	/// <exception cref="ArgumentNullException">
	/// <paramref name="value"/> or <paramref name="policy"/> is
	/// <see langword="null"/>.
	/// </exception>
	...
	private static string ReplaceControls(
		...
		for ( var index = 0; index < value.Length; ++index ) {
			...
			if ( !IsControlOrInvalid( character ) ) {
				builder?.Append( character );
				continue;
			}
			builder ??= new StringBuilder( value.Length ).Append(
				value,
				0,
				index
			);
	...
	private static string QuoteForShellEscape(
		...
		if ( HasControlOrInvalid( value ) ) {
			return string.Concat(
				"$'",
				EscapeCContent( value, true ),
				"'"
			);
		}
		return QuoteForShell( value, always );
	}
	...
	private static string QuoteAsC(
		...
		var content = EscapeCContent( value, false );
		return quote
	...
	private static string QuoteForLocale(
		...
		var content = EscapeCContent( value, true );
			...
			switch ( character ) {
				...
				default:
					if ( char.IsSurrogate( character ) ) {
						...
						builder.Append( '\\' );
						builder.Append(
							Convert.ToString( (int)character, 8 )!.PadLeft( 3, '0' )
						);
					} else {
						builder.Append( character );
					}
