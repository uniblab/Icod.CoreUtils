		...
		try {
			...
			for ( var byteCount = 128; byteCount <= 131072; byteCount *= 2 ) {
				...
				if ( error != invalidArgument ) {
					return HostResourceValue<ProcessorAffinityDescriptor>.Unavailable(
						string.Concat( "sched_getaffinity failed with native error ", error.ToString( CultureInfo.InvariantCulture ), "." ),
						HostResourceProvenance.NativeOperatingSystem
					);
				}
			}
	...
	private static HostResourceValue<ProcessorAffinityDescriptor> TryReadWindowsAffinity() {
		try {
			...
			if ( !NativeMethods.GetProcessAffinityMask( process, out var processMask, out _ ) ) {
				return HostResourceValue<ProcessorAffinityDescriptor>.Unavailable(
					string.Concat( "GetProcessAffinityMask failed with native error ", Marshal.GetLastPInvokeError().ToString( CultureInfo.InvariantCulture ), "." ),
					HostResourceProvenance.WindowsProcessorGroup
				);
			}
			...
			return HostResourceValue<ProcessorAffinityDescriptor>.Available(
				new ProcessorAffinityDescriptor( ids.Select( static value => (long)value ), isComplete: groupCount <= 1 ),
				HostResourceProvenance.WindowsProcessorGroup,
				groupCount <= 1
					? null
	...
	private static HostResourceValue<ProcessorQuotaDescriptor> TryReadWindowsJobQuota(
		...
		try {
			...
			if ( !NativeMethods.IsProcessInJob( process, IntPtr.Zero, out var inJob ) ) {
				return HostResourceValue<ProcessorQuotaDescriptor>.Unavailable(
					string.Concat( "IsProcessInJob failed with native error ", Marshal.GetLastPInvokeError().ToString( CultureInfo.InvariantCulture ), "." ),
			...
			if (
				...
				return HostResourceValue<ProcessorQuotaDescriptor>.Unavailable(
					string.Concat( "QueryInformationJobObject failed with native error ", Marshal.GetLastPInvokeError().ToString( CultureInfo.InvariantCulture ), "." ),
					HostResourceProvenance.WindowsJobObject
				);
			}
		...
		try {
			...
			var descriptor = new ProcessorTopologyDescriptor(
				packages.Count > 0
					...
					: HostResourceValue<int>.Unavailable( "sysfs did not expose processor package identifiers.", HostResourceProvenance.LinuxSysFileSystem ),
				cores.Count > 0
					? HostResourceValue<int>.Available( cores.Count, HostResourceProvenance.LinuxSysFileSystem )
					: HostResourceValue<int>.Unavailable( "sysfs did not expose physical core identifiers.", HostResourceProvenance.LinuxSysFileSystem ),
				logical,
				numaCount > 0
					? HostResourceValue<int>.Available( numaCount, HostResourceProvenance.LinuxSysFileSystem )
					: HostResourceValue<int>.Unavailable( "sysfs did not expose NUMA node directories.", HostResourceProvenance.LinuxSysFileSystem )
			);
		...
		try {
			...
			try {
				if (
					...
					return HostResourceValue<ProcessorTopologyDescriptor>.Unavailable(
						string.Concat( "GetLogicalProcessorInformationEx failed with native error ", Marshal.GetLastPInvokeError().ToString( CultureInfo.InvariantCulture ), "." ),
						HostResourceProvenance.NativeOperatingSystem
					);
				}
				var packages = 0;
				...
				uint offset = 0;
				while ( offset < length ) {
					var relationship = (NativeMethods.LogicalProcessorRelationship)Marshal.ReadInt32( buffer, checked((int)offset) );
				...
				var descriptor = new ProcessorTopologyDescriptor(
					packages > 0
						...
						: HostResourceValue<int>.Unavailable( "Processor packages were not reported.", HostResourceProvenance.NativeOperatingSystem ),
					cores > 0
						? HostResourceValue<int>.Available( cores, HostResourceProvenance.NativeOperatingSystem )
						: HostResourceValue<int>.Unavailable( "Physical cores were not reported.", HostResourceProvenance.NativeOperatingSystem ),
					online,
					numaNodes > 0
						? HostResourceValue<int>.Available( numaNodes, HostResourceProvenance.NativeOperatingSystem )
						: HostResourceValue<int>.Unavailable( "NUMA nodes were not reported.", HostResourceProvenance.NativeOperatingSystem )
				);
				return HostResourceValue<ProcessorTopologyDescriptor>.Available(
					descriptor,
					HostResourceProvenance.NativeOperatingSystem
...
#pragma warning disable SYSLIB1054 // Keep native host-resource adapters self-contained and compatible with the existing project settings.
	private static class NativeMethods {
		...
		/// <returns><see langword="true"/> on success.</returns>
		[DllImport( "kernel32.dll", EntryPoint = "GetProcessDefaultCpuSets", ExactSpelling = true, SetLastError = true )]
		[return: MarshalAs( UnmanagedType.Bool )]
		...
		/// <returns><see langword="true"/> on success.</returns>
		[DllImport( "kernel32.dll", EntryPoint = "GetLogicalProcessorInformationEx", ExactSpelling = true, SetLastError = true )]
		[return: MarshalAs( UnmanagedType.Bool )]
