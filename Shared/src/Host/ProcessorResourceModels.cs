...
public sealed record ProcessorAffinityDescriptor {
	...
	public ProcessorAffinityDescriptor(
		...
		if ( identifiers.Length == 0 ) {
			throw new ArgumentException( "An affinity descriptor requires at least one processor identifier.", nameof( processorIdentifiers ) );
		}
		if ( identifiers[0] < 0 ) {
			throw new ArgumentOutOfRangeException( nameof( processorIdentifiers ), "Processor identifiers cannot be negative." );
		}
...
public sealed record ProcessorQuotaDescriptor {
	...
	/// <exception cref="ArgumentException">Only one of quota and period is present, or <paramref name="scope"/> is empty.</exception>
	public ProcessorQuotaDescriptor(
		...
		if (
			...
			throw new ArgumentOutOfRangeException( nameof( quotaMicroseconds ), "Quota and period values must be positive." );
		}
...
public sealed record ProcessorResourceSnapshot {
	...
	private IReadOnlyList<HostResourceCapability> BuildCapabilities() {
		...
		return [
			...
			CreateCapability( HostResourceCapabilityKind.ProcessAvailableProcessorCount, ProcessAvailableProcessorCount ),
