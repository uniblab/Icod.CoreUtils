...
- `TrByteEscapeParser.cs` parses the low-level named and one-to-three-digit octal byte escapes used by GNU `tr` set parsing. It retains whether each byte was escaped and reproduces GNU's deterministic warning behavior for trailing backslashes and overflowing three-digit octal escapes.
