...
`Shared.FileSystem` contains the Coreutils-specific filesystem policy that remains after neutral host-filesystem mechanism moved to `Icod.CommandFramework`. Coreutils-local code now concentrates on GNU copy/move policy, ownership handling, mode parsing, and filesystem-usage reporting. Neutral durable-flush, sparse-allocation, file-clone, traversal, metadata, creation-mask observation, mutation, recursive-mutation, and transactional-replacement mechanisms are consumed from the published framework package.
...
| Whole-file copy-on-write clone | `CloneFileAsync(source, destination)` |
...
| Whole-file clone | Unsupported | `FICLONE` where the filesystem supports it | Unsupported | Unsupported |
...
- `cp`/`mv` keep GNU reflink selection policy locally and delegate the host clone attempt to `CloneFileAsync`.
