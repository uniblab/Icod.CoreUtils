...
GNU-derived fixtures must identify their upstream test or documentation origin in `tests/Patch.Tests/fixtures/README.md`. Repository tests must not depend on a locally installed `patch` executable and must not shell out to GNU patch during ordinary test execution.
