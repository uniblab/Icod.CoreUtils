...
**sha256sum** — compute or check SHA-256 message digests
...
`Icod.CoreUtils.Sha256Sum` is a managed .NET implementation of GNU Coreutils `sha256sum(1)`, modeled on GNU Coreutils 9.11.

The command computes a SHA-256 message digest for each `FILE` and writes a checksum record to standard output. If no file is specified, or if a file operand is `-`, input is read from standard input. The command produces a 256-bit SHA-256 digest.
...
GNU `sha256sum` was written by Ulrich Drepper, Scott Miller, and David Madore.
...
`sha256sum(1)`, `cksum(1)`, `b2sum(1)`, `md5sum(1)`, `sha1sum(1)`, `sha256sum(1)`, `sha512sum(1)`