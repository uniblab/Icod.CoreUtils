...
public static class Command {
	...
	public static ValueTask<int> RunAsync( string[] args, CommandContext? context = null ) {
		return RunAsync(
			args,
			context ?? CommandContext.CreateConsole( "ln" ),
		...
		try {
			...
			if ( parsing.Version ) {
				await context.StandardOutput.WriteLineAsync(
					"ln (Icod.CoreUtils) 0.1"
				).ConfigureAwait( false );
				return 0;
			}
			if ( parsing.Error is not null ) {
				await context.StandardError.WriteLineAsync(
					string.Concat( "ln: ", parsing.Error )
				).ConfigureAwait( false );
				await context.StandardError.WriteLineAsync(
			...
			if ( operands.Count == 0 ) {
				return await OperandFailureAsync(
					context,
					"missing file operand"
				).ConfigureAwait( false );
			}
			if ( parsing.Options.TargetDirectory is null && operands.Count == 1 ) {
				return await OperandFailureAsync(
					context,
					string.Concat(
			...
			var destinationIsDirectory = !parsing.Options.NoTargetDirectory
				&& IsDirectoryTarget(
					destination,
					parsing.Options.NoDereference
				);
			...
			foreach ( var source in sources ) {
				var linkPath = ( destinationIsDirectory )
					? System.IO.Path.Combine( destination, Basename( source ) )
					: destination
				;
	...
	private static async ValueTask<bool> CreateOneAsync(
		...
		if ( DestinationExists( linkPath ) ) {
			if ( options.Interactive ) {
				await context.StandardError.WriteAsync(
					string.Concat( "ln: replace ", Quote( linkPath ), "? " )
				).ConfigureAwait( false );
				var answer = await context.StandardInput.ReadLineAsync().ConfigureAwait( false );
				if ( string.IsNullOrEmpty( answer ) || char.ToLowerInvariant( answer[ 0 ] ) != 'y' ) {
					return true;
				}
			...
			if ( options.Backup ) {
				...
				try {
					MovePhysical( linkPath, backup );
				} catch ( Exception ex ) when ( ex is IOException or UnauthorizedAccessException ) {
					await FailureAsync( context, linkPath, ex.Message ).ConfigureAwait( false );
					return false;
				}
			} else {
				var removal = await mutationProvider.RemoveFileAsync(
					linkPath,
				...
				if ( !removal.Succeeded ) {
					await FailureAsync(
						context,
						linkPath,
						removal.Message ?? "cannot remove destination"
					).ConfigureAwait( false );
					return false;
				}
			}
		...
		if ( options.Symbolic ) {
			var targetText = ( options.Relative )
				? MakeRelativeTarget( source, linkPath )
				: source
			;
			...
			var dereference = ( options.Physical )
		...
		if ( options.Verbose ) {
			await context.StandardOutput.WriteLineAsync(
				string.Concat(
					...
					" -> ",
					Quote(
						( options.Symbolic && options.Relative )
							? MakeRelativeTarget( source, linkPath )
							: source
					)
				)
	...
	private static (Options Options, List<string> Operands, string? Error, bool Help, bool Version) Parse(
		string[] args
	) {
		var symbolic = false;
		var force = false;
		...
		var verbose = false;
		var relative = false;
		var noDereference = false;
		var logical = true;
		var physical = false;
		var backup = false;
		var control = "existing";
		var suffix = "~";
		...
		for ( var i = 0; i < args.Length; i++ ) {
			...
			if ( arg == "--help" ) {
				return ( default!, operands, null, true, false );
			}
			if ( arg == "--version" ) {
				return ( default!, operands, null, false, true );
			}
			if ( arg is "-s" or "--symbolic" ) {
				...
				verbose = true;
			} else if ( arg is "-r" or "--relative" ) {
				relative = true;
			} else if ( arg is "-n" or "--no-dereference" ) {
				noDereference = true;
			} else if ( arg is "-L" or "--logical" ) {
				logical = true;
				physical = false;
			} else if ( arg is "-P" or "--physical" ) {
				physical = true;
				logical = false;
			} else if ( arg is "-b" or "--backup" ) {
				backup = true;
			} else if ( arg.StartsWith( "--backup=", StringComparison.Ordinal ) ) {
				backup = true;
				control = arg[ 9.. ];
			} else if ( arg is "-T" or "--no-target-directory" ) {
				noTargetDirectory = true;
			} else if ( arg is "-t" or "--target-directory" ) {
				if ( ++i >= args.Length ) {
					return (
						default!,
						operands,
			...
			} else if ( arg.StartsWith( "--target-directory=", StringComparison.Ordinal ) ) {
				targetDirectory = arg[ 19.. ];
			} else if ( arg is "-S" or "--suffix" ) {
				if ( ++i >= args.Length ) {
					return (
						default!,
						operands,
				...
				return (
					default!,
					operands,
					string.Concat( "unrecognized option ", Quote( arg ) ),
					false,
					false
				);
			}
		...
		if ( relative && !symbolic ) {
			return (
				default!,
				operands,
				"--relative can only be used with --symbolic",
				false,
				false
			);
		}
	...
	public static async ValueTask WriteUsageAsync(
		...
		foreach ( var line in new[] {
			...
			"  -n, --no-dereference        treat a destination symlink to a directory as a file",
	...
	private static bool IsDirectoryTarget( string path, bool noDereference ) {
		try {
			var info = new DirectoryInfo( path );
			return info.Exists && !( noDereference && info.LinkTarget is not null );
		} catch {
			return false;
		}
	}
	...
	private static string Basename( string path ) => System.IO.Path.GetFileName(
		path.TrimEnd(
	...
	private static string MakeRelativeTarget( string source, string linkPath ) {
		var sourceFull = System.IO.Path.GetFullPath( source );
		var parent = System.IO.Path.GetDirectoryName(
			System.IO.Path.GetFullPath( linkPath )
		) ?? Directory.GetCurrentDirectory();
		return System.IO.Path.GetRelativePath( parent, sourceFull );
	}

	private static string ChooseBackupName( string path, string control, string suffix ) {
		if (
			control is "numbered" or "t"
			|| (
				control is "existing" or "nil"
				&& File.Exists( string.Concat( path, ".~1~" ) )
			)
		) {
			for ( var n = 1; ; n++ ) {
	...
	private static string Describe( FileSystemMutationResult result ) => result.ErrorCode switch {
		...
		FileSystemMutationErrorCode.WrongObjectKind => "hard link not allowed for directory",
	...
	private static async ValueTask FailureAsync(
		CommandContext context,
		string path,
		string reason
	) {
	...
	private static async ValueTask<int> OperandFailureAsync(
		CommandContext context,
		string message
	) {
		await context.StandardError.WriteLineAsync(
			string.Concat( "ln: ", message )
		).ConfigureAwait( false );
		await context.StandardError.WriteLineAsync(
			"Try 'ln --help' for more information."
		).ConfigureAwait( false );
