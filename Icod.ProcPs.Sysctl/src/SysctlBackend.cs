...
public interface ISysctlBackend {
	...
	/// <param name="key">The normalized key relative to <c>/proc/sys</c>.</param>
	/// <param name="cancellationToken">The cancellation token.</param>
	/// <returns>The value without terminal line separators.</returns>
	...
	/// <param name="dryRun">Whether the operation should validate but not modify the kernel parameter.</param>
	/// <param name="cancellationToken">The cancellation token.</param>
	...
	/// <param name="cancellationToken">The cancellation token.</param>
	/// <returns>The observable keys.</returns>
	...
	/// <param name="pattern">The configuration path or glob.</param>
	/// <param name="cancellationToken">The cancellation token.</param>
	/// <returns>The matching files, or the literal path when no wildcard is present or no wildcard match exists.</returns>
	...
	/// <param name="cancellationToken">The cancellation token.</param>
	/// <returns>The matching files in that directory.</returns>
	...
	/// <param name="path">The file path.</param>
	/// <param name="cancellationToken">The cancellation token.</param>
	/// <returns>The configuration lines.</returns>
...
internal sealed class SystemSysctlBackend : ISysctlBackend {
	...
	/// <summary>Gets the singleton production backend.</summary>
	...
	public async Task<string> ReadValueAsync( string key, CancellationToken cancellationToken = default ) {
		...
		try {
			...
			throw new SysctlBackendException( SysctlBackendFailureKind.PermissionDenied, "Permission denied", exception );
		} catch ( IOException exception ) {
			throw ClassifyIoException( exception );
		}
	}
	...
	public Task<IReadOnlyList<string>> EnumerateConfigurationFilesAsync( string directory, CancellationToken cancellationToken = default ) {
		...
		try {
			var files = Directory.EnumerateFiles( directory, "*.conf", SearchOption.TopDirectoryOnly )
				.OrderBy( static path => GetPortableFileName( path ), StringComparer.Ordinal )
				.ToArray();
	...
	public async Task<IReadOnlyList<string>> ReadConfigurationFileAsync( string path, CancellationToken cancellationToken = default ) {
		...
		try {
			...
			throw new SysctlBackendException( SysctlBackendFailureKind.NotFound, "No such file or directory", exception );
		} catch ( DirectoryNotFoundException exception ) {
			throw new SysctlBackendException( SysctlBackendFailureKind.NotFound, "No such file or directory", exception );
		} catch ( UnauthorizedAccessException exception ) {
			throw new SysctlBackendException( SysctlBackendFailureKind.PermissionDenied, "Permission denied", exception );
		} catch ( IOException exception ) {
			throw new SysctlBackendException( SysctlBackendFailureKind.IoFailure, exception.Message, exception );
		}
	}
	...
	private static string ResolveProcPath( string key ) {
		...
		if ( key.StartsWith( "/", StringComparison.Ordinal ) || key.Contains( '\0' ) ) {
			throw new SysctlBackendException( SysctlBackendFailureKind.NotFound, "No such file or directory" );
		}
